...
function PlanPrice({
  ...
  const amount = price.monthly === 0 ? null : annual ? price.annual : price.monthly;
  ...
  return (
    <div className="mb-2">
      <div className="flex items-start gap-0.5">
        {amount !== null && (
          <span
            className={`text-xl font-light mt-2 ${isPrime ? "text-background" : "text-slate-400"
              }`}
          >
            $
          </span>
...
export function PlanCard({ plan, annual }: PlanCardProps) {
  ...
  return (
    <Card
      className={`relative overflow-hidden flex flex-col transition-all duration-300 hover:-translate-y-2 ${isPrime
    ...
    >
      ...
      <CardHeader className="relative z-10 pb-0 pt-7 px-7">
        <div className="flex items-start justify-between mb-5">
          <span
            className={`text-2xl font-light tracking-tight ${isPrime ? "text-background" : "text-slate-900"
              }`}
            style={{ fontFamily: "'Georgia', serif" }}
          >
            {name}
        ...
        <p
          className={`text-sm italic leading-relaxed mb-5 ${isPrime ? "text-background/80" : "text-slate-400"
            }`}
        >
          {description}
        </p>
      ...
      <CardContent className="relative z-10 flex flex-col flex-1 pt-5 px-7 pb-7">
        <p
          className={`text-[10px] font-bold tracking-widest uppercase mb-4 ${isPrime ? "text-background" : "text-slate-400"
            }`}
        >
          What's included
        </p>
        ...
        <Button
          ...
          className={`w-full font-semibold text-sm py-5 rounded-xl transition-all duration-200 ${isPrime
